...
public class VentasController(RegistroGanaderoDbContext db) : Controller
{
    public async Task<IActionResult> Index(int pagina = 1, CancellationToken ct = default)
    {
        var vm = await db.Ventas.AsNoTracking().Include(v => v.Animal)
            .OrderByDescending(v => v.FechaVenta).ToListaPaginadaAsync(pagina, ct);
        return View(vm);
    }
    ...
    [Authorize(Roles = RoleNames.Operacion)]
    public async Task<IActionResult> Create(Venta model, string? animalKey, CancellationToken ct = default)
    {
    ...
    public async Task<IActionResult> Edit(int? id, CancellationToken ct = default)
    {
        ...
        var item = await db.Ventas.FindAsync([id], ct);
    ...
    [HttpPost, ValidateAntiForgeryToken]
    [Authorize(Roles = RoleNames.Operacion)]
    public async Task<IActionResult> Edit(int id, Venta model, string? animalKey, CancellationToken ct = default)
    {
        if (id != model.IdVenta) return NotFound();
        AplicarAnimalKey(model, animalKey);
        if (!ModelState.IsValid) { await CargarAnimalesAsync(ct, animalKey); return View(model); }
        db.Update(model);
        await db.SaveChangesAsync(ct);
    ...
    {
        if (id is null) return NotFound();
        var item = await db.Ventas.AsNoTracking().Include(v => v.Animal)
    ...
    {
        var item = await db.Ventas.FindAsync([id], ct);
        if (item is not null) { db.Ventas.Remove(item); await db.SaveChangesAsync(ct); }
        return RedirectToAction(nameof(Index));
    }
    ...
    private static void AplicarAnimalKey(Venta model, string? animalKey)
    {
